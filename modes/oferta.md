...
Classify the offer into one of the 6 archetypes (see `_shared.md`). If it's a hybrid, note the 2 closest. This determines:
...
Section on **gaps** with a mitigation strategy for each. For each gap:
...
2. **"Sell senior without lying" plan**: archetype-adapted phrasing, concrete achievements to highlight, how to position founder experience as an advantage
...
Table with data and cited sources. If there's no data, say so instead of making it up.
...
**Story Bank:** If `interview-prep/story-bank.md` exists, check whether any of these stories are already there. If not, append new ones. Over time this builds a reusable bank of 5–10 master stories that can be adapted to any interview question.
...
- **Evergreen/continuous hire postings:** If the JD explicitly says "ongoing" or "rolling," note it as context — this is not a ghost job, it's a pipeline role.
